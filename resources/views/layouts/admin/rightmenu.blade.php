<aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="index3.html" class="brand-link">
        ...
        <span class="brand-text font-weight-light">بیگ مارکت وی آی پی</span>
    </a>
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                @foreach($menu1 as $item)
                    ...
                    @endif
                    ">
                    <a href="{{($item->link?route(''.$item->link.''):route('admindashboard'))}}" class="nav-link
                   @if(\Request::route()->getName() == $item->link)
                        active
                    @endif
                    ">
                    ...
                    <ul class="nav nav-treeview">
                        ...
                        @foreach($menu2 as $item2)
                            ...
                            @endif
                                ">
                            <a href="{{($item2->link?route(''.$item2->link.''):route('admindashboard'))}}" class="nav-link
                            @if(\Request::route()->getName() == $item->link)
                                active
                            @endif
                            ">
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                <li class="nav-item">
                                    <a href="{{($item3->link?route(''.$item3->link.''):route('admindashboard'))}}" class="nav-link
                                    @if(\Request::route()->getName() == $item->link)
                                        active
                                    @endif
                                        ">
